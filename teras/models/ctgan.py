...
class CTGANGenerator(keras.Model):
    ...
    Args:
        data_dim: ``int``,
            ...
            You can access the dimensionailty of the transformed dataset
            throught the ``.data_dim`` attribute of the
            ``CTGANDataSampler`` instance used in sampling the dataset.

        metadata: ``dict``,
            The ``CTGANGeneratorBlock``, applies different activation
            functions to its outputs depending on the type of features
            (categorical or numerical).
            And to determine the feature types and for other computations
            during the activation step, the ``metadata`` computed during
            the data transformation step, is required.

            It can be accessed through the ``.get_metadata()`` method of
            the ``CTGANDataTransformer`` instance which was used to
            transform the raw input data.

            Note that, this is NOT the same metadata as
            ``features_metadata``, which is computed using the
            ``get_features_metadata_for_embedding`` utility function from
            ``teras.utils``.
            You must use the ``.get_metadata()`` method of the
            ``CTGANDataTransformer`` to access it.
    ...
    def __init__(self,
        ...
        if not isinstance(units_values, (list, tuple)):
            raise ValueError(
                f"""`units_values` must be a list or tuple of units which 
                determines the number of Generator residual blocks and 
                the dimensionality of those blocks.
                Received: {type(units_values)}""")

        if metadata is None:
            raise ValueError(
                f"""`metadata` cannot be None.
                `metadata`, which is computed during the data 
                transformation step, is required by the `CTGANGenerator` 
                to apply relevant activation functions to the output of 
                the Generator. But received `None`.
                Please pass the metadata by accessing it through the 
                `.get_metadata()` method of the `CTGANDataTransformer` 
...
class CTGANDiscriminator(keras.Model):
    ...
    Args:
        ...
        gradient_penalty_lambda: ``float``, default 10,
                ...
                lambda value is directly proportional to the strength of
                gradient penalty.
    ...
    def __init__(self,
        ...
        if not isinstance(units_values, (list, tuple)):
            raise ValueError(
                ...
                dimensionality of those blocks.
    ...
    if backend() == "tensorflow":
        ...
        def gradient_penalty(self,
            ...
            Args:
                trainable_variables: Discriminator's trainable variables
                non_trainable_variables: Discriminator's non-trainable
                    variables
                real_samples: Data samples drawn from the real dataset
                generated_samples: Data samples generated by the generator
...
class CTGAN(_CTGAN_LF):
    ...
    Args:
        ...
        metadata: ``dict``,
            A dictionary of metadata computed during the data
            transformation step.
            Simply pass the result of ``.get_metadata()`` method of the
            ``CTGANDataTransformer`` instance which was used to
            transform the raw input data.
            The Generator in ``CTGAN`` architecture applies different
            activation functions to the output of Generator, depending
            on the type of features.
            And to determine the feature types and for other computation
            during activation step, the ``metadata`` computed during the
            data transformation step, is required.
        ...
        num_discriminator_steps: ``int``, default 1,
            Number of discriminator training steps per ``CTGAN`` training
            step.

        latent_dim: ``int``, default 128,
            Dimensionality of noise or `z` that serves as input to
            Generator to generate samples.
        ...
        packing_degree: ``int``, default 8,
            ...
            [Diederik P. Kingma et al.] architecture,
            which proposes passing ``m`` samples at once to discriminator
            instead of ``1`` to be jointly classified as real or fake by
            the discriminator in order to tackle the issue of mode
            collapse inherent in the GAN based architectures.
            The number of samples passed jointly ``m``, is termed as the
            ``packing degree``.

        gradient_penalty_lambda: ``float``, default 10,
            Controls the strength of gradient penalty in the
            ``CTGANDiscriminator``.
            lambda value is directly proportional to the strength of
            gradient penalty.
            Gradient penalty penalizes the discriminator for large weights
            in an attempt to combat Discriminator becoming too confident
            and overfitting.
    """
    def __init__(self,
                 input_dim: int,
        ...
        if metadata is None:
            raise ValueError(
                ...
                "You can access the `metadata` through `.get_metadata()` "
                "method of `CTGANDataTransformer` instance.")
