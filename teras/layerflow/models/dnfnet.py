...
class DNFNet(keras.Model):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        if isinstance(dnnf_layers, (keras.layers.Layer,
                                    keras.models.Model)):
            # keep it as is
            ...
            raise ValueError("`dnnf_layers` can either be a list of "
                             "`DNNF` layers, a Keras Layer, "
                             f"or a Keras model model but received type: "
                             f"{type(dnnf_layers)}.")
