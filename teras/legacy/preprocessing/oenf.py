...
class PiecewiseLinearEncoding:
    ...
    def fit(self,
        ...
        for feature_idx in range(X.shape[1]):
            ...
            if self.method.lower() == "quantile":
                feature_n_bins = min(self.n_bins,
                                     tf.unique(feature)[0].shape[0])
                quantiles = \
                tf.unique(tfp.stats.quantiles(feature, feature_n_bins))[0]
                self.bin_edges.append(quantiles.numpy())

            elif self.method.lower() == "tree":
                assert y is not None, (
                "For tree based encoding, y must not be None. Given y: ",
                y)
                if isinstance(y, str):
                ...
                decision_tree = decision_tree.fit(
                    feature.values.reshape(-1, 1), y).tree_
                tree_thresholds = []
                for node_id in range(decision_tree.node_count):
                    ...
                    if decision_tree.children_left[node_id] != \
                            decision_tree.children_right[node_id]:
                        tree_thresholds.append(
                            decision_tree.threshold[node_id])
                    tree_thresholds.append(feature.min())
                    tree_thresholds.append(feature.max())
                    self.bin_edges.append(
                        np.array(sorted(set(tree_thresholds))))
    ...
    def transform(self,
        ...
        for feature_idx in range(X.shape[1]):
            feature = X.iloc[:, feature_idx]
            feature_bin_edges = np.r_[
                -np.inf, self.bin_edges[feature_idx][1:-1], np.inf]
            self.bins.append(
                tf.cast(
                    tf.searchsorted(
                        feature_bin_edges,
            ...
            if self.encoding == 'binary':
                ...
                feature_bin_sizes = self.bin_edges[feature_idx][1:] - \
                                    self.bin_edges[feature_idx][:-1]
                feature_bins = self.bins[feature_idx]

                self.bins_values.append(
                    (feature - self.bin_edges[feature_idx][feature_bins]) /
                    feature_bin_sizes[feature_bins]
                )
