...
from teras.legacy.utils.vime import vime_mask_generator, vime_pretext_generator
...
class VimeDataTransformer(_BaseDataTransformer):
    ...
    def transform(self, x: pd.DataFrame):
        if self.categorical_features is not None:
            x[self.categorical_features] = self.one_hot_encoder(
                x[self.categorical_features])

        if self.numerical_features is not None:
            x[self.numerical_features] = np.sum(x[self.numerical_features],
                                                axis=1)
        return x
...
class VimeDataSampler:
    ...
    def get_dataset(self,
        ...
        semi_labeled_ds = (tf.data.Dataset
                           .from_tensor_slices(
            {"x_labeled": labeled_data, "y_labeled": y},
        ...
        if num_samples_labeled > num_samples_unlabeled:
            num_times_to_repeat = tf.cast(
                tf.math.ceil(num_samples_labeled / num_samples_unlabeled),
                dtype=tf.int64)
            semi_unlabeled_ds = semi_unlabeled_ds.repeat(
                num_times_to_repeat)
