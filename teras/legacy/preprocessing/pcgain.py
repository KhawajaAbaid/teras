...
import numpy as np
import pandas as pd
from teras.legacy.preprocessing.gain import (
    GAINDataTransformer as _GAINDataTransformer,
...
class PCGAINDataSampler(_GAINDataSampler):
    ...
    def get_pretraining_dataset(self,
        ...
        missing_rate_per_row = np.sum(np.isnan(x_transformed),
                                      axis=1) / dim
        ...
        pretraining_dataset = tf.data.Dataset.from_generator(
            ...
            tf.TensorSpec(shape=(None, dim), dtype=tf.float32,
                          name="generator_pretraining_data"),
            tf.TensorSpec(shape=(None, dim), dtype=tf.float32,
                          name="discriminator_pretraining_data")
            ),
            args=[x_transformed, gen_idx, disc_idx, batch_size]
        )
        return pretraining_dataset

    def _pretraining_dataset_generator(self, x, gen_idx, disc_idx,
                                       batch_size):
        """
