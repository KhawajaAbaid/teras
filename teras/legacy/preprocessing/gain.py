...
from teras.legacy.preprocessing.base.base_data_transformer import \
    BaseDataTransformer as _BaseDataTransformer
...
class GAINDataTransformer(_BaseDataTransformer):
    ...
    def fit(self, x,
        ...
        if self.categorical_features is not None:
            ...
            x_temp[self.categorical_features] = self.encoder.transform(
    ...
    def transform(self,
        ...
        if not isinstance(x, pd.DataFrame):
            raise ValueError(
                "Only pandas dataframe is supported by DataTransformation class."
        ...
        if self.categorical_features is not None:
            x[self.categorical_features] = self.encoder.transform(
                x[self.categorical_features])
    ...
    def reverse_transform(self,
        ...
        if self.min_vals is None and self.max_vals is None:
            raise ValueError(
                "The values for `min_vals` and `max_vals` do not exist. "
        ...
        if self.categorical_features is not None:
            x[self.categorical_features] = self.encoder.inverse_transform(
                x[self.categorical_features])
...
class GAINDataSampler:
    ...
    def generator(self, x):
        ...
        for i in range(steps_per_epoch):
            ...
            if i == (
                    steps_per_epoch - 1) and not is_n_divisible_by_batch_size:
                ...
                x_discriminator = x[
                    discriminator_idx[from_index: to_index]]
