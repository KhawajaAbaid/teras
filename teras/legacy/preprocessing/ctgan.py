...
from teras.legacy.utils import tf_random_choice
import numpy as np
import pandas as pd
from teras.legacy.preprocessing.base import BaseDataTransformer
from teras.legacy.utils.types import FeaturesNamesType
    ...
    def fit(self, x):
        ...
        for feature_name in self.numerical_features:
            ...
            def random_choice(probs_array):
                ...
                return np.random.choice(np.arange(len(probs_array)),
                                        p=probs_array)
            ...
            # Means
            clusters_means = self.bay_guass_mix.means_.squeeze()[
                valid_clusters_indicator]
            self.metadata[feature_name]['clusters_means'] = clusters_means
            # Standard Deviations
            clusters_stds = \
            np.sqrt(self.bay_guass_mix.covariances_).squeeze()[
                valid_clusters_indicator]
            self.metadata[feature_name]['clusters_stds'] = clusters_stds

        self.metadata["relative_indices_all"] = np.array(
            relative_indices_all)
    ...
    def reverse_transform(self, x_normalized):
        ...
        for feature_name in self.numerical_features:
            ...
            if isinstance(x_normalized, pd.DataFrame):
                ...
                x[feature_name] = normalized_feature * (
                            self.std_multiplier * stds) + means
                ...
                raise ValueError(
                    f"`x_normalized` must be either a pandas DataFrame or numpy ndarray. "
...
class CTGANDataTransformer(BaseDataTransformer):
    ...
    def transform_categorical_data(self, x):
        ...
        for feature_name in self.categorical_features:
            ...
            probs = np.around(
                np.array(list(categories_probs_dict.values())), 7)
            # Normalizing so all probs sum up to 1
            probs = probs / np.sum(probs)
            categories, categories_probs = list(
                categories_probs_dict.keys()), probs
            categories_probs_all.append(categories_probs)
            categories_all.append(categories)
        categorical_metadata["total_num_categories"] = sum(
            num_categories_all)
        categorical_metadata["num_categories_all"] = num_categories_all
        categorical_metadata["relative_indices_all"] = np.array(
            relative_indices_all)
    ...
    def transform(self, x, **kwargs):
        ...
        if self.num_numerical_features > 0:
            x_numerical = self.transform_numerical_data(
                x[self.numerical_features])
            self.metadata[
                "numerical"] = self.mode_specific_normalizer.metadata
            total_transformed_features += (
                        self.metadata["numerical"]["relative_indices_all"][
                            -1] +
                        self.metadata["numerical"][
                            "num_valid_clusters_all"][-1])
        if self.num_categorical_features > 0:
            x_categorical = self.transform_categorical_data(
                x[self.categorical_features])
            total_transformed_features += (self.metadata["categorical"][
                                               "relative_indices_all"][
                                               -1] +
    ...
    def reverse_transform(self, x_generated):
        ...
        if self.num_numerical_features > 0:
            num_valid_clusters_all = self.metadata["numerical"][
                "num_valid_clusters_all"]
        if self.num_categorical_features > 0:
            num_categories_all = self.metadata["categorical"][
                "num_categories_all"]
        data = {}
        ...
        for index, feature_name in enumerate(all_features):
            ...
            if index < len(self.numerical_features):
                ...
                betas = x_generated[:,
                        index + 1: index + 1 + num_valid_clusters_all[
                            cont_index]]
                ...
                means = self.metadata["numerical"][feature_name][
                    "clusters_means"]
                ...
                stds = self.metadata["numerical"][feature_name][
                    "clusters_stds"]
                ...
                raw_feature_parts = x_generated[:,
                                    index: index + num_categories_all[
                                        cat_index]]
...
class CTGANDataSampler:
    ...
    def get_dataset(self,
        ...
        row_idx_raw = [x_original.groupby(feature).groups for feature in
        ...
        total_num_categories = self.metadata["categorical"][
        ...
        dataset = tf.data.Dataset.from_generator(
            ...
            output_signature=(tf.TensorSpec(
                ...
                name="real_samples"),
                              tf.TensorSpec(shape=(
                              self.batch_size, total_num_categories),
                                            dtype=tf.float32,
                                            name="cond_vectors_real"),
                              tf.TensorSpec(shape=(
                              self.batch_size, total_num_categories),
                                            dtype=tf.float32,
                                            name="cond_vectors"),
                              tf.TensorSpec(shape=(self.batch_size,
    ...
    def sample_cond_vectors_for_training(self,
        ...
        cond_vectors = tf.zeros([self.batch_size,
                                 self.metadata["categorical"][
                                     "total_num_categories"]])
        ...
        random_features_relative_indices = tf.gather(
        ...
        random_values_idx_offsetted = random_values_idx + tf.cast(
            random_features_relative_indices,
            dtype=tf.int32)
        ...
        indices_cond = tf.stack(
            [tf.range(self.batch_size), random_values_idx_offsetted],
            axis=1)
        ones = tf.ones(self.batch_size)
        cond_vectors = tf.tensor_scatter_nd_update(cond_vectors,
                                                   indices_cond, ones)
        # indices_mask = list(zip(tf.range(batch_size), tf.cast(random_features_idx, dtype=tf.int32)))
        indices_mask = tf.stack(
            [tf.range(self.batch_size), random_features_idx], axis=1)
        masks = tf.tensor_scatter_nd_update(masks, indices_mask,
    ...
    def sample_cond_vectors_for_generation(self, batch_size):
        ...
        num_categories_all = self.metadata["categorical"][
            "num_categories_all"]
        cond_vectors = tf.zeros((batch_size, self.metadata["categorical"][
            "total_num_categories"]))
        ...
        random_values_idx = [tf.squeeze(tf.random.uniform(shape=(1,),
                                                          ...
                                                          dtype=tf.int32))
                             for num_categories in
                             random_num_categories_all]
        ...
        random_features_relative_indices = tf.gather(
            ...
            indices=random_features_idx)
        random_values_idx_offsetted = random_values_idx + tf.cast(
            random_features_relative_indices, dtype=tf.int32)
        # random_values_idx_offsetted = tf.cast(random_values_idx_offsetted, dtype=tf.int32)
        indices_cond = list(
            zip(tf.range(batch_size), random_values_idx_offsetted))
    ...
    def generator(self, x_transformed, for_tvae=False):
        ...
        for _ in range(num_steps_per_epoch):
            random_features_idx = tf.random.uniform([self.batch_size],
                                                    minval=0,
                                                    maxval=len(
                                                        self.categorical_features),
                                                    dtype=tf.int32)
            ...
            random_features_categories_probs = tf.gather(
                ...
                indices=random_features_idx)
            random_values_idx = [
                tf_random_choice(np.arange(len(feature_probs)),
                                 n_samples=1,
                                 p=feature_probs)
                for feature_probs in random_features_categories_probs]
            random_values_idx = tf.cast(tf.squeeze(random_values_idx),
                                        dtype=tf.int32)
            ...
            shuffled_features_idx = tf.gather(random_features_idx,
                                              indices=shuffled_idx)
            shuffled_values_idx = tf.gather(random_values_idx,
                                            indices=shuffled_idx)
            sample_idx = []
            ...
            for feat_id, val_id in zip(shuffled_features_idx,
                ...
                s_id = tf_random_choice(
                    self.row_idx_by_categories[tf.squeeze(feat_id)][
                        tf.squeeze(val_id)],
                    n_samples=1)
            ...
            cond_vectors_real = tf.gather(cond_vectors,
                                          indices=shuffled_idx)
