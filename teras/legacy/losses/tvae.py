...
def elbo_loss_tvae(data_dim,
    ...
    for i, relative_index in enumerate(meta_data.relative_indices_all):
        ...
        if i < len(meta_data.numerical.num_valid_clusters_all):
            ...
            eq = real_samples[:, relative_index] - tf.nn.tanh(
                generated_samples[:, relative_index])
            loss_temp = tf.reduce_sum((eq ** 2 / 2 / (std ** 2)))
            loss.append(loss_temp)
            loss_temp = tf.math.log(std) * tf.cast(
                tf.shape(real_samples)[0], dtype=tf.float32)
            loss.append(loss_temp)

            # calculate betas loss
            num_clusters = meta_data.numerical.num_valid_clusters_all[
                cont_i]
            logits = generated_samples[:,
                     relative_index + 1: relative_index + 1 + num_clusters]
            labels = tf.argmax(real_samples[:,
                               relative_index + 1: relative_index + 1 + num_clusters],
                               axis=-1)
            cross_entropy_loss = cross_entropy(y_pred=logits,
                                               y_true=labels)
            loss.append(cross_entropy_loss)
            cont_i += 1
        else:
            num_categories = meta_data.categorical.num_categories_all[
                cat_i]
            logits = generated_samples[:,
                     relative_index: relative_index + num_categories]
            labels = tf.argmax(real_samples[:,
                               relative_index: relative_index + num_categories],
                               axis=-1)
            cross_entropy_loss = cross_entropy(y_pred=logits,
