...
def info_nce_loss(real_projection_outputs=None,
    ...
    logits_ab = tf.matmul(real_projection_outputs,
                          augmented_projection_outputs,
                          transpose_b=True) / temperature
    logits_ba = tf.matmul(augmented_projection_outputs,
                          real_projection_outputs,
                          transpose_b=True) / temperature
    loss_a = losses.SparseCategoricalCrossentropy(from_logits=True)(
        y_true=labels,
        y_pred=logits_ab)
    loss_b = losses.SparseCategoricalCrossentropy(from_logits=True)(
        y_true=labels,
        y_pred=logits_ba)
...
def denoising_loss(real_samples=None,
    ...
    if categorical_features_metadata is None:
        raise ValueError(
            "`categorical_features_metadata` cannot be None. ")

    num_features = tf.shape(real_samples)[1]
    num_categorical_features = len(categorical_features_metadata)
    num_categories_per_feature = list(
        map(lambda x: len(x[1]), categorical_features_metadata.values()))
    ...
    for current_idx, (feature_idx, _) in enumerate(
            categorical_features_metadata.values()):
        num_categories = num_categories_per_feature[current_idx]
        loss += tf.keras.losses.SparseCategoricalCrossentropy(
            from_logits=True)(
            y_true=real_samples[:, feature_idx],
            # real_samples have the original feature order
            y_pred=reconstructed_samples[:,
                   current_idx: current_idx + num_categories])
        current_idx += 1
    ...
    mse_loss = tf.cond(pred=num_categorical_features < num_features,
                       true_fn=lambda: tf.reduce_sum(tf.losses.mse(
                           real_samples[:, num_categorical_features:],
