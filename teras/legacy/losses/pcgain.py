...
from teras.legacy.losses.gain import (
    generator_loss as generator_pretraining_loss,
    discriminator_loss)
...
def generator_loss(real_samples=None,
    ...
    cross_entropy_loss = -tf.reduce_mean(
        (1 - mask) * tf.math.log(discriminator_pred + 1e-8))
    mse_loss = losses.MSE(y_true=(mask * real_samples),
                          y_pred=(mask * generated_samples))
    info_entropy_loss = -tf.reduce_mean(
        classifier_pred * tf.math.log(classifier_pred + 1e-8))
    loss = cross_entropy_loss + (alpha * mse_loss) + (
                beta * info_entropy_loss)
    return loss
