...
def generator_loss(generated_samples,
    ...
    numerical_features_relative_indices = metadata["numerical"][
    ...
    loss = tf.reduce_sum(loss * tf.cast(mask, dtype=tf.float32)) / tf.cast(
        tf.shape(y_generated)[0], dtype=tf.float32)
