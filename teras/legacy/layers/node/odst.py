...
from teras.legacy.utils import sparsemoid
from teras.legacy.activations import sparsemax
import tensorflow_probability as tfp
from teras.legacy.utils import get_activation


@keras.saving.register_keras_serializable(package="teras.legacy.layers.node")
class ObliviousDecisionTree(keras.layers.Layer):
    ...
    def __init__(self,
        ...
        self.choice_function = sparsemax if choice_function is None else get_activation(
            choice_function)
        self.bin_function = sparsemoid if bin_function is None else get_activation(
            bin_function)
    ...
    def build(self, input_shape):
        ...
        self.response = self.add_weight(
            initializer=self.response_initializer,
            shape=(self.num_trees, self.tree_dim, 2 ** self.depth))
        self.feature_selection_logits = self.add_weight(
            initializer=self.selection_logits_initializer,
            shape=(input_dim, self.num_trees, self.depth))
        self.feature_thresholds = tf.Variable(
            initial_value=keras.initializers.zeros()(
        ...
        bin_codes = tf.reshape(indices, shape=(1, -1)) // tf.reshape(
            offsets, shape=(-1, 1)) % 2
    ...
    def _data_aware_initialization(self, inputs, eps=1e-6):
        ...
        feature_selectors = self.choice_function(
            self.feature_selection_logits, axis=0)
        feature_values = tf.einsum('bi,ind->bnd', inputs,
                                   feature_selectors)
        beta_dist = tfp.distributions.Beta(self.threshold_init_beta,
                                           self.threshold_init_beta)
        percentiles_q = 100 * beta_dist.sample(
            [self.num_trees * self.depth])

        flattened_feature_values = tf.map_fn(tf.keras.backend.flatten,
                                             feature_values)

        feature_thresholds = tfp.stats.percentile(flattened_feature_values,
                                                  percentiles_q)
        feature_thresholds = tf.reshape(feature_thresholds,
                                        shape=(self.num_trees, self.depth))
        temperatures = tfp.stats.percentile(
            tf.abs(feature_values - feature_thresholds),
        ...
        log_tempratures = tf.reshape(log_tempratures,
                                     shape=[self.num_trees, self.depth])
    ...
    def call(self, inputs):
        if len(inputs.shape) > 2:
            return self.call(tf.reshape(
                tf.reshape(inputs, shape=(-1, inputs.shape[-1])),
        ...
        feature_values = tf.einsum('bi,ind->bnd', inputs,
                                   features_selectors)
        threshold_logits = (
                                       feature_values - self.feature_thresholds) * tf.exp(
            -self.log_temperatures)
        threshold_logits = tf.stack([-threshold_logits, threshold_logits],
                                    axis=-1)
        bins = self.bin_function(threshold_logits)
        bin_matches = tf.einsum('btds,dcs->btdc', bins,
                                self.bin_codes_1hot)
        response_weights = tf.reduce_prod(bin_matches, axis=-2)
        response = tf.einsum('bnd,ncd->bnc', response_weights,
                             self.response)
