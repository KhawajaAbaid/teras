...
from teras.legacy.layers.tabnet import TabNetFeatureTransformer
...
class TabNetDecoder(keras.layers.Layer):
    ...
    def __init__(self,
        ...
        for i in range(self.num_decision_steps):
            self.features_transformers_per_step.append(
            ...
            self.projection_layers_per_step.append(
    ...
    def call(self, inputs, mask=None):
        ...
        reconstructed_features = tf.zeros(
            shape=(batch_size, self.data_dim))

        for i in range(self.num_decision_steps):
            feat_output = self.features_transformers_per_step[i](inputs)
            reconstructed_features += self.projection_layers_per_step[i](
                feat_output)
