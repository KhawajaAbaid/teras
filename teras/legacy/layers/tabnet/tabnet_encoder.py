...
@keras.saving.register_keras_serializable("teras.legacy.layers.tabnet")
class TabNetEncoder(keras.layers.Layer):
    ...
    def __init__(self,
        ...
        self.inputs_norm = keras.layers.BatchNormalization(
        ...
        self.attentive_transformers_per_step = [
            TabNetAttentiveTransformer(data_dim=self.data_dim,
                                       batch_momentum=self.batch_momentum,
                                       virtual_batch_size=self.virtual_batch_size,
    ...
    def call(self, inputs, mask=None):
        ...
        outputs_aggregated = tf.zeros(
            shape=(batch_size, self.decision_step_output_dim))
        ...
        for step_num in range(self.num_decision_steps):
            ...
            x = self.features_transformers_per_step[step_num](
                masked_features)

            if step_num > 0 or self.num_decision_steps == 1:
                decision_step_outputs = self.relu(
                    x[:, :self.decision_step_output_dim])
                ...
                self.feature_importances_per_sample.append(
                    feature_importances_per_sample)

            features_for_attentive_transformer = (
            x[:, self.decision_step_output_dim:])

            if step_num < self.num_decision_steps - 1:
                # Determines the feature masks via linear and nonlinear
                # transformations, taking into account of aggregated feature use
                mask_values = self.attentive_transformers_per_step[
                    step_num](features_for_attentive_transformer,
                ...
                total_entropy += tf.reduce_mean(
                    tf.reduce_sum(-mask_values * tf.math.log(
                        mask_values + self.epsilon), axis=1)
                ) / (self.num_decision_steps - 1)
                entropy_loss = total_entropy

                # Feature Selection
                masked_features = tf.multiply(mask_values,
                                              normalized_inputs)
            else:
                entropy_loss = 0.
