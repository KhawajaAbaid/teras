...
from teras.legacy.utils.types import Number


@keras.saving.register_keras_serializable(package="teras.legacy.layers")
class CategoricalFeatureEmbedding(keras.layers.Layer):
    ...
    def __init__(self,
        ...
        if not len(features_metadata["categorical"]) > 0:
            raise ValueError(
                "`features_metadata` does not contain any categorical features. "
        ...
        self.categorical_features_metadata = features_metadata[
            "categorical"]
        self.lookup_tables, self.embedding_layers = self._get_lookup_tables_and_embedding_layers()
        self._num_categorical_features = len(
            self.categorical_features_metadata)

    def _get_lookup_tables_and_embedding_layers(self):
        """Lookup tables and embedding layers for each categorical feature"""
        lookup_tables = []
        embedding_layers = []
        for feature_name, (feature_idx,
                           vocabulary) in self.categorical_features_metadata.items():
            ...
            if self.encode:
                if isinstance(vocabulary[0], Number):
                    ...
                    lookup = keras.layers.IntegerLookup(
                    ...
                    raise TypeError(
                        "`CategoricalFeatureEmbedding` layer can only encode values of type int "
    ...
    def call(self, inputs):
        ...
        categorical_feature_embeddings = tf.TensorArray(
            size=self._num_categorical_features,
            dtype=tf.float32)
        ...
        for feature_name, (
        feature_idx, _) in self.categorical_features_metadata.items():
            ...
            categorical_feature_embeddings = categorical_feature_embeddings.write(
                current_idx, feature)
            current_idx += 1
        categorical_feature_embeddings = categorical_feature_embeddings.stack()
        categorical_feature_embeddings = tf.squeeze(
            categorical_feature_embeddings, axis=2)
        categorical_feature_embeddings = tf.transpose(
            categorical_feature_embeddings, perm=[1, 0, 2])
        categorical_feature_embeddings.set_shape(
            (None, self._num_categorical_features, self.embedding_dim))
