...
class PeriodicEmbedding(keras.layers.Layer):
    ...
    def __init__(self,
        ...
        if initialization.lower() not in ["normal", "log-linear"]:
            raise ValueError(
                "Invalid value for initialization. Must be one of ['log-linear', 'normal']\n"
        ...
        self._numerical_features_indices = list(
    ...
    def build(self, input_shape):
        if self.initialization == "log-linear":
            ...
            self.coefficients = tf.random.normal(
                shape=(self._num_numerical_features, self.n),
                mean=0.,
                stddev=self.sigma)
