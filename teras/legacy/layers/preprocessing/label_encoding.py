...
@keras.saving.register_keras_serializable(
    package="teras.legacy.layers.preprocessing")
class LabelEncoding(keras.layers.Layer):
    ...
    def _get_lookup_tables(self):
        ...
        lookup_tables = []
        for feature_name, (feature_idx, vocabulary) in \
        self.features_metadata["categorical"].items():
            ...
            if isinstance(vocabulary[0], Number):
                ...
                raise TypeError(
                    "`LabelEncoding` layer can only encode values of type int "
    ...
    def call(self, inputs):
        if self.concatenate_numerical_features:
            ...
            encoded_features = tf.TensorArray(
        ...
        for feature_idx in self._categorical_features_idx:
            ...
            encoded_features = encoded_features.write(
                index=feature_idx if (self.concatenate_numerical_features
        ...
        if self.concatenate_numerical_features:
            for i, feature_idx in enumerate(self._numerical_features_idx):
                feature = tf.cast(
                    tf.expand_dims(inputs[:, feature_idx], 1), tf.float32)
                encoded_features = encoded_features.write(
                    index=feature_idx if self.keep_features_order else current_idx,
                    value=feature)
                current_idx += 1
        ...
        if self.concatenate_numerical_features:
            ...
            encoded_features.set_shape(
                (None, self._num_categorical_features))
