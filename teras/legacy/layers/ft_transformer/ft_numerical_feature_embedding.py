...
@keras.saving.register_keras_serializable(
    package="teras.legacy.layers.ft_transformer")
class FTNumericalFeatureEmbedding(keras.layers.Layer):
    ...
    def __init__(self,
        ...
        if not len(features_metadata["numerical"]) > 0:
            raise ValueError(
                "`features_metadata` contains no numerical features. "
        ...
        self.numerical_features_metadata = self.features_metadata[
            "numerical"]
        self.embedding_dim = embedding_dim

        self._num_numerical_features = len(
            self.numerical_features_metadata)
        self._numerical_features_idx = list(
            self.numerical_features_metadata.values())
    ...
    def call(self, inputs):
        numerical_feature_embeddings = tf.TensorArray(
            size=self._num_numerical_features,
            dtype=tf.float32)
        ...
        for i, feature_idx in enumerate(self._numerical_features_idx):
            ...
            numerical_feature_embeddings = numerical_feature_embeddings.write(
                i, feature)

        numerical_feature_embeddings = tf.squeeze(
            numerical_feature_embeddings.stack())
        numerical_feature_embeddings = tf.transpose(
            numerical_feature_embeddings, perm=[1, 0, 2])
        numerical_feature_embeddings.set_shape(
            (None, self._num_numerical_features, self.embedding_dim))
