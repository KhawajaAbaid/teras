...
from teras.legacy.utils.types import ActivationType, InitializerType


@keras.saving.register_keras_serializable(package="teras.legacy.layers.gain")
class GAINGeneratorBlock(keras.layers.Layer):
    ...
    def get_config(self):
        ...
        if not isinstance(self.activation, str):
            activation_serialized = keras.layers.deserialize(
                self.activation)

        kernel_initializer_serialized = self.kernel_initializer
        if not isinstance(self.activation, str):
            kernel_initializer_serialized = keras.layers.deserialize(
