...
class DNFNetLocalization(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.mu = tf.Variable(initial_value=mu_initializer(
            shape=(self.num_formulas, input_dim)),
                              shape=(self.num_formulas, input_dim),
                              name='exp_mu')
        sigma_initializer = keras.initializers.random_normal()
        self.sigma = tf.Variable(initial_value=sigma_initializer(
    ...
    def call(self, inputs):
        diff = tf.expand_dims(inputs, axis=1) - tf.expand_dims(self.mu,
                                                               axis=0)
