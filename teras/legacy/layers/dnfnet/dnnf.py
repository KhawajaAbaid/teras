...
from teras.legacy.utils.dnfnet import (compute_total_number_of_literals,
...
from teras.legacy.utils.types import (IntegerSequence,
                               FloatSequence)


@keras.saving.register_keras_serializable(package="teras.legacy.layers.dnfnet")
class DNNF(keras.layers.Layer):
    ...
    def __init__(self,
                 ...
                 keep_feature_prob_arr: FloatSequence = [0.1, 0.3, 0.5,
                                                         0.7, 0.9],
        ...
        self.localization = DNFNetLocalization(
    ...
    def build(self, input_shape):
        ...
        self.feature_selection = DNFNetFeatureSelection(
            input_dim=input_dim,
            num_formulas=self.num_formulas,
        ...
        self.total_number_of_literals = compute_total_number_of_literals(
            self.num_formulas,
            self.num_conjunctions_arr,
            self.conjunctions_depth_arr)
        num_literals_per_formula_arr = compute_num_literals_per_formula(
            self.num_conjunctions_arr,
            self.conjunctions_depth_arr)
        total_number_of_conjunctions = compute_total_number_of_conjunctions(
            self.num_formulas,
            self.num_conjunctions_arr)
        ...
        self.bias = tf.Variable(name='literals_bias',
                                shape=tf.shape(self.literals_random_mask)[
                                    1],
                                initial_value=tf.fill(dims=(
                                tf.shape(self.literals_random_mask)[1],),
                                                      value=0.))
        self.weight = self.add_weight(name='literals_weights',
                                      shape=tf.shape(
                                          self.literals_random_mask),
        ...
        b = tf.Variable(
            tf.cast(tf.zeros(tf.shape(self.literals_random_mask)[1]),
                    dtype=tf.bool),
            trainable=False)
        conjunctions_indicator_matrix = create_conjunctions_indicator_matrix(
            b,
            total_number_of_conjunctions,
            self.conjunctions_depth_arr,
            )
        conjunctions_indicator_matrix = tf.cast(
            conjunctions_indicator_matrix, dtype=tf.float32)
        self.conjunctions_indicator_sparse_matrix = dense_ndim_array_to_sparse_tensor(
            conjunctions_indicator_matrix)
        ...
        formulas_indicator_matrix = create_formulas_indicator_matrix(c,
                                                                     self.num_formulas,
                                                                     self.num_conjunctions_arr)
        formulas_indicator_matrix = tf.cast(formulas_indicator_matrix,
                                            dtype=tf.float32)
        self.formulas_indicator_sparse_matrix = dense_ndim_array_to_sparse_tensor(
    ...
    def call(self, inputs):
        ...
        out_literals = tf.tanh(tf.add(tf.matmul(x, tf.multiply(self.weight,
                                                               self.learnable_binary_mask)),
                                      self.bias))

        # Soft Conjunctions
        out_conjunctions = and_operator(tf_matmul_sparse_dense(
            self.conjunctions_indicator_sparse_matrix, out_literals),
                                        d=self.and_bias)
        out_DNNFs = or_operator(
            tf_matmul_sparse_dense(self.formulas_indicator_sparse_matrix,
                                   out_conjunctions),
            d=self.or_bias)
