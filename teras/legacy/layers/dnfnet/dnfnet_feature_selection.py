...
from teras.legacy.utils.dnfnet import (tf_matmul_sparse_dense,
                                generate_random_mask,
                                extension_matrix,
                                binary_threshold)
from teras.legacy.utils.types import FloatSequence
...
class DNFNetFeatureSelection(keras.layers.Layer):
    ...
    def __init__(self,
        ...
        self.v_temp = tf.Variable(tf.zeros(self.num_formulas),
                                  trainable=False)
    ...
    def build(self, input_shape):
        ...
        self.learnable_mask = tf.Variable(
            initial_value=tf.fill(dims=(self.input_dim, self.num_formulas),
    ...
    def call(self,
        ...
        literals_random_mask, formulas_random_mask = generate_random_mask(
            self.input_dim,
            self.keep_feature_prob_arr,
            num_literals_per_formula_arr,
            self.num_formulas)
        num_effective_features = tf.reduce_sum(formulas_random_mask,
                                               axis=0)
        formulas_random_mask = tf.reshape(formulas_random_mask,
                                          shape=tf.shape(
                                              self.learnable_mask))
        ext_matrix = extension_matrix(self.v_temp,
                                      self.num_formulas,
                                      num_literals_per_formula_arr)
        learnable_mask_01 = binary_threshold(self.learnable_mask,
                                             eps=self.binary_threshold_eps)

        l2_square_norm_selected = tf.linalg.diag_part(
            tf.matmul(tf.transpose(tf.square(self.learnable_mask)),
                      formulas_random_mask))
        l1_norm_selected = tf.linalg.diag_part(
            tf.matmul(tf.transpose(tf.abs(self.learnable_mask)),
                      formulas_random_mask))

        l2 = tf.abs(
            tf.divide(l2_square_norm_selected,
                      num_effective_features) - self.elastic_net_beta * self.binary_threshold_eps ** 2)
        l1 = tf.abs(tf.divide(l1_norm_selected,
                              num_effective_features) - self.elastic_net_beta * self.binary_threshold_eps)
        elastic_net_reg = tf.reduce_mean(
            (l2 * ((1 - tf.nn.sigmoid(
                self.elastic_net_alpha)) / 2) + l1 * tf.nn.sigmoid(
                self.elastic_net_alpha)))
        learnable_binary_mask = tf_matmul_sparse_dense(ext_matrix,
                                                       learnable_mask_01)
