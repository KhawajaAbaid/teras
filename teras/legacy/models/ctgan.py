...
from teras.legacy.layerflow.models.ctgan import CTGAN as _CTGAN_LF
from teras.legacy.layers.activation import GumbelSoftmax
from teras.legacy.utils.types import UnitsValuesType


@keras.saving.register_keras_serializable(package="teras.legacy.models")
class CTGANGenerator(keras.Model):
    ...
    Args:
        ...
        metadata: ``dict``,
            ...
            using the ``get_features_metadata_for_embedding`` utility function from ``teras.legacy.utils``.
            ...
            You can import the ``CTGANDataTransformer`` as follows,
                >>> from teras.legacy.preprocessing import CTGANDataTransformer
        ...
        units_values: ``List[int]`` or ``Tuple[int]``, default [256, 256],
            ...
            You can access the ``CTGANGeneratorBlock`` as follows,
                >>> from teras.legacy.layers import CTGANGeneratorBlock
    """
    ...
    def apply_activations_by_feature_type(self, interim_outputs):
        ...
        """
        outputs = []
        numerical_features_relative_indices = self.metadata["numerical"][
            "relative_indices_all"]
        features_relative_indices_all = self.metadata[
            "relative_indices_all"]
        ...
        for i, index in enumerate(features_relative_indices_all):
            ...
            if i < len(numerical_features_relative_indices):
                ...
                betas = self.gumbel_softmax(interim_outputs[:,
                                            index + 1: index + 1 +
                                                       num_valid_clusters_all[
                                                           cont_i]])
            ...
            # elif index in categorical_features_relative_indices:
                ...
                ds = self.gumbel_softmax(interim_outputs[:,
...
class CTGANDiscriminator(keras.Model):
    ...
    Args:
        units_values: ``List[int]`` or ``Tuple[int]``, default [256, 256],
            ...
            You can import the ``CTGANDiscriminatorBlock`` as follows,
                >>> from teras.legacy.layers import CTGANDiscriminatorBlock
    ...
    def gradient_penalty(self,
        ...
        gradient_penalty = tf.reduce_mean(
            tf.square(gradients_norm - 1.0)) * self.gradient_penalty_lambda
