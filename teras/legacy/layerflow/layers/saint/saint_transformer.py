...
@keras.saving.register_keras_serializable(
    package="teras.legacy.layerflow.layers.saint")
class SAINTTransformer(keras.layers.Layer):
    ...
    Args:
        multi_head_inter_sample_attention: ``keras.layers.Layer``,
            ...
            You can import this layer as follows,
                >>> from teras.legacy.layers import MultiHeadInterSampleAttention
    ...
    def get_config(self):
        ...
        new_config = {
            'multi_head_inter_sample_attention': keras.layers.serialize(
                self.multi_head_inter_sample_attention),
    ...
    def from_config(cls, config):
        multi_head_inter_sample_attention = keras.layers.deserialize(
            config.pop("multi_head_inter_sample_attention"))
        feed_forward = keras.layers.deserialize(config.pop("feed_forward"))
        transformer = keras.layers.deserialize(config.pop("transformer"))
        return cls(
            multi_head_inter_sample_attention=multi_head_inter_sample_attention,
