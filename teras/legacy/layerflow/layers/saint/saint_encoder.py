...
from teras.legacy.utils import (serialize_layers_collection,
                         deserialize_layers_collection)
from typing import List, Union

LAYERS_COLLECTION = Union[
    keras.layers.Layer, List[keras.layers.Layer], keras.models.Model]
...
class SAINTEncoder(keras.layers.Layer):
    ...
    Args:
        saint_transformer_layers: ``keras.layers.Layer`` or ``List[Layer]`` or ``models.Model``,
            ...
            You can import the ``SAINTTransformer`` layer as follows,
                >>> from teras.legacy.layerflow.layers import SAINTTransformer
    """
    ...
    def get_config(self):
        ...
        config.update({
                          'saint_transformer_layers': serialize_layers_collection(
                              self.saint_transformer_layers)})
        return config

    @classmethod
    def from_config(cls, config):
        saint_transformer_layers = deserialize_layers_collection(
            config.pop("saint_transformer_layers"))
