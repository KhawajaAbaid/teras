...
from teras.legacy.layerflow.layers.common.common import HiLOL
...
@keras.saving.register_keras_serializable(
    package="teras.legacy.layerflow.layers.common")
...
class Transformer(keras.layers.Layer):
    ...
    Args:
        ...
        feed_forward: ``keras.layers.Layer``,
            ...
            You can import the ``FeedForward`` layer as follows,
                >>> from teras.legacy.layerflow.layers import FeedForward
    ...
    def get_config(self):
        ...
        new_config = {'multi_head_attention': keras.layers.serialize(
            self.multi_head_attention),
                      'feed_forward': keras.layers.serialize(
                          self.feed_forward),
                      'layer_norm_1': keras.layers.deserialize(
                          self.layer_norm_1),
                      'layer_norm_2': keras.layers.deserialize(
                          self.layer_norm_2)
                      }
    ...
    def from_config(cls, config):
        multi_head_attention = keras.layers.deserialize(
            config.pop("multi_head_attention"))
...
class Encoder(keras.layers.Layer):
    ...
    Args:
        transformer_layers: ``List[keras.layers.Layer]``,
            ...
            You can import the ``Transformer`` layer as follows,
                >>> from teras.legacy.layerflow.layers import Transformer
    """
    ...
    def get_config(self):
        config = super().get_config()
        config.update({'transformer_layers': keras.layers.serialize(
            self.transformer_layers)})
        return config

    @classmethod
    def from_config(cls, config):
        transformer_layers = keras.layers.deserialize(
            config.pop("transformer_layers"))
