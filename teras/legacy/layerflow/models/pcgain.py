...
from teras.legacy.utils.pcgain import cluster
import tensorflow_probability as tfp
from teras.legacy.losses.pcgain import (generator_loss,
...
class PCGAIN(keras.Model):
    ...
    Args:
        ...
        classifier: ``keras.Model``,
            ...
            You can import the ``PCGAINClassifier`` as follows,
                >>> from teras.legacy.models import PCGAINClassifier
    ...
    def __init__(self,
        ...
        if not hasattr(pretrainer, "generator"):
            raise ValueError(
                "The `pretrainer` doesn't have a `generator` attribute or property. \n"
        ...
        if not hasattr(pretrainer, "discriminator"):
            raise ValueError(
                "The `pretrainer` doesn't have a `discriminator` attribute or property. \n"
        ...
        self.z_sampler = tfp.distributions.Uniform(low=0.,
                                                   high=0.01,
                                                   name="z_sampler")
        self.hint_vectors_sampler = tfp.distributions.Binomial(
            total_count=1,
    ...
    def pretrain(self,
        ...
        """
        print(
            "Pretrain Step 1/4: Pretraining the generator and discriminator")
        ...
        self.classifier.fit(imputed_data, y=pseudo_labels,
    ...
    def train_step(self, data):
        if self._first_batch:
            if not self._pretrained:
                raise AssertionError(
                    "The model has not yet been pretrained. "
        ...
        for _ in range(self.num_discriminator_steps):
            # Create mask
            mask = tf.constant(1.) - tf.cast(tf.math.is_nan(x_disc),
                                             dtype=tf.float32)
            # replace nans with 0.
            ...
            # Sample hint vectors
            hint_vectors = self.hint_vectors_sampler.sample(
                sample_shape=(tf.shape(x_disc)))
            hint_vectors *= mask
            ...
            with tf.GradientTape() as tape:
                discriminator_pred = self.discriminator(
                    tf.concat([x_hat_disc, hint_vectors], axis=1))
                loss_disc = self.discriminator_loss(discriminator_pred,
                                                    mask)
            gradients = tape.gradient(loss_disc,
                                      self.discriminator.trainable_weights)
            self.discriminator_optimizer.apply_gradients(
                zip(gradients, self.discriminator.trainable_weights))

        # =====> Train the Generator <=====
        mask = tf.constant(1.) - tf.cast(tf.math.is_nan(x_gen),
                                         dtype=tf.float32)
        x_gen = tf.where(tf.math.is_nan(x_gen), x=0., y=x_gen)
        z = self.z_sampler.sample(sample_shape=tf.shape(x_gen))
        hint_vectors = self.hint_vectors_sampler.sample(
            sample_shape=(tf.shape(x_gen)))
        hint_vectors *= mask
        ...
        with tf.GradientTape() as tape:
            ...
            disc_preds = self.discriminator(
                tf.concat([x_hat, hint_vectors], axis=1))
        ...
        gradients = tape.gradient(loss_gen,
        ...
        results.update({"generator_" + m.name: m.result() for m in
                        self.generator.metrics})
        results.update({"discriminator_" + m.name: m.result() for m in
    ...
    def predict_step(self, data):
        ...
        # Create mask
        mask = tf.constant(1.) - tf.cast(tf.math.is_nan(data),
                                         dtype=tf.float32)
    ...
    def impute(self,
        ...
        if reverse_transform:
            if data_transformer is None:
                raise ValueError(
                    "To reverse transform the raw imputed data, `data_transformer` must not be None. "
