...
from teras.legacy.losses.saint import info_nce_loss, denoising_loss


@keras.saving.register_keras_serializable(
    package="teras.legacy.layerflow.models.backbones")
class SAINT(keras.Model):
    ...
    Args:
        ...
        encoder: ``keras.layers.Layer``,
            ...
            You can import the ``SAINTEncoder`` layer as follows,
                >>> from teras.legacy.layerflow.layers import SAINTEncoder
        ...
        categorical_feature_embedding: ``keras.layers.Layer``,
            ...
            You can import the ``CategoricalFeatureEmbedding`` layer as follows,
                >>> from teras.legacy.layers import CategoricalFeatureEmbedding
        ...
        numerical_feature_embedding: ``keras.layers.Layer``,
            ...
            You can import the ``SAINTNumericalFeatureEmbedding`` layer as follows,
                >>> from teras.legacy.layers import SAINTNumericalFeatureEmbedding
        ...
        head: ``keras.layers.Layer``,
            ...
            You can import the ``ClassificationHead`` and ``RegressionHead`` layers as follows,
                >>> from teras.legacy.layers import ClassificationHead
                >>> from teras.legacy.layers import RegressionHead
    """
    ...
    def __init__(self,
        ...
        if categorical_feature_embedding is None and numerical_feature_embedding is None:
            raise ValueError(
                "Both `categorical_feature_embedding` and `numerical_feature_embedding` "
                "cannot be None at the same time as a tabular dataset must contains "
                "features of at least one of the types if not both. ")

        if encoder is None:
            raise ValueError(
                "`encoder` cannot be None. Please pass an instance of `SAINTEncoder` layer. "
                "You can import it as, `from teras.legacy.layerflow.layers import SAINTEncoder``")
    ...
    def get_config(self):
        ...
        config.update({'input_dim': self.input_dim,
                       'categorical_feature_embedding': keras.layers.serialize(
                           self.categorical_feature_embedding),
                       'numerical_feature_embedding': keras.layers.serialize(
...
@keras.saving.register_keras_serializable(package="teras.legacy.layerflow.models")
class SAINTPretrainer(keras.Model):
    ...
    Args:
        ...
        features_metadata: ``dict``,
            ...
            You can get this dictionary from
                >>> from teras.legacy.utils import get_features_metadata_for_embedding
        ...
        mixup: ``keras.layers.Layer``,
            ...
            You can import the ``MixUp`` layer as follows,
                >>> from teras.legacy.layers import MixUp
        ...
        cutmix: ``keras.layers.Layer``,
            ...
            You can import the `CutMix` layer as follows,
                >>> from teras.legacy.layers import CutMix
        ...
        projection_head_1: ``keras.layers.Layer``,
            ...
            You can import the ``SAINTProjectionHead`` layer as follows,
                >>> from teras.legacy.layerflow.layers import SAINTProjectionHead
        ...
        reconstruction_head: ``keras.layers.Layer``,
            ...
            You can import the ``SAINTReconstructionHead`` as follows,
                >>> from teras.legacy.layerflow.layers import SAINTReconstructionHead
    ...
    def __init__(self,
        ...
        if mixup is None:
            raise ValueError(
                "`mixup` cannot be None. Please pass an instance of `MixUp` layer. "
                "You can import it as, `from teras.legacy.layers import MixUp`")

        if cutmix is None:
            raise ValueError(
                "`cutmix` cannot be None. Please pass an instance of `CutMix` layer. "
                "You can import it as, `from teras.legacy.layers import CutMix``")
        ...
        if projection_head_1 is None:
            raise ValueError("`projection_head_1` cannot be None. "
                             ...
                             "You can import it as, `from teras.legacy.layers import SAINTProjectionHead``")
        ...
        if reconstruction_head is None:
            raise ValueError("`reconstruction_head` cannot be None. "
                             ...
                             "You can import it as, `from teras.legacy.layers import SAINTReconstructionHead`")
    ...
    def call(self, inputs):
        ...
        if self._numerical_features_exist:
            ...
            numerical_features_prime = self.model.numerical_feature_embedding(
                x_prime)
            if p is not None:
        ...
        tf.shape(z)[0], tf.reduce_prod(tf.shape(z)[1:])))
        z_prime = tf.reshape(z_prime, shape=(
        tf.shape(z_prime)[0], tf.reduce_prod(tf.shape(z_prime)[1:])))
