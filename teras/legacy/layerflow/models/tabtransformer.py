...
class TabTransformer(keras.Model):
    ...
    Args:
        ...
        categorical_feature_embedding: ``layers.Layer``,
            ...
            You can import the ``CategoricalFeatureEmbedding`` layer as follows,
                >>> from teras.legacy.layerflow.layers import CategoricalFeatureEmbedding
        ...
        column_embedding: ``layers.Layer``,
            ...
            You can import the ``TabTColumnEmbedding`` layer as follows,
                >>> from teras.legacy.layers import TabTransformerColumnEmbedding
        ...
        encoder: ``layers.Layer``,
            ...
            You can import the ``Encoder`` layer as follows,
                >>> from teras.legacy.layerflow.layers import Encoder
        ...
        numerical_feature_normalization: ``layers.Layer``,
            ...
            You can import the ``NumericalFeatureNormalization`` layer as follows,
                >>> from teras.legacy.layerflow.layers import NumericalFeatureNormalization
    ...
    def __init__(self,
        ...
        if categorical_feature_embedding is None and numerical_feature_normalization is None:
            raise ValueError(
                "Both `categorical_feature_embedding` and `numerical_feature_normalization` "
        ...
        if categorical_feature_embedding is not None:
            ...
            if encoder is None:
                raise ValueError(
                    "`encoder` is required to encode the categorical embedding, "
                    "but received `None`. "
                    "Please pass an instance of `Encode` layer. "
                    "You can import it as, `from teras.legacy.layerflow.layers import Encoder`")
            x = encoder(x)
    ...
    def get_config(self):
        ...
        config.update({'input_dim': self.input_dim,
                       ...
                       'numerical_feature_normalization': keras.layers.serialize(
...
class TabTransformerPretrainer(keras.Model):
    ...
    def __init__(self,
        ...
        self.num_features = len(
            self.features_metadata["categorical"]) + len(
            self.features_metadata["numerical"])
        self.num_features_to_replace = tf.cast(
            tf.cast(self.num_features, tf.float32) * self.replace_rate,
            dtype=tf.int32)
    ...
    def train_step(self, data):
        ...
        feature_indices_to_replace = tf.random.uniform(
            (tf.shape(data)[0], self.num_features_to_replace),
