...
from teras.legacy.losses.ctgan import generator_loss, discriminator_loss
...
class CTGAN(keras.Model):
    ...
    Args:
        generator: ``keras.Model``,
            ...
            You can import the ``CTGANGenerator`` as follows,
                >>> from teras.legacy.models import CTGANGenerator
        ...
        discriminator: ``keras.Model``,
            ...
            You can import the ``CTGANDiscriminator`` as follows,
                >>> from teras.legacy.models import CTGANDiscriminator
    ...
    def train_step(self, data):
        ...
        for _ in range(self.num_discriminator_steps):
            ...
            generated_samples = tf.concat(
                [generated_samples, cond_vectors], axis=1)
            real_samples = tf.concat([real_samples, cond_vectors_real],
                                     axis=1)

            with tf.GradientTape(persistent=True) as tape:
                y_generated = self.discriminator(generated_samples)
                y_real = self.discriminator(real_samples)
                grad_pen = self.discriminator.gradient_penalty(
            ...
            self.discriminator_optimizer.apply_gradients(
                zip(gradients_pen, self.discriminator.trainable_weights))
            self.discriminator_optimizer.apply_gradients(
                zip(gradients_loss, self.discriminator.trainable_weights))
        ...
        generator_results = {'generator_' + m.name: m.result() for m in
                             self.generator.metrics}
        results.update(generator_results)
        discriminator_results = {'discriminator_' + m.name: m.result() for
                                 m in self.discriminator.metrics}
    ...
    def generate(self,
        ...
        for _ in tqdm(range(num_steps), desc="Generating Data"):
            ...
            cond_vector = data_sampler.sample_cond_vectors_for_generation(
                batch_size)
            input_gen = tf.concat([z, cond_vector], axis=1)
            generated_samples.append(
                self.generator(input_gen, training=False))
    ...
    def from_config(cls, config):
        ...
        discriminator = keras.layers.deserialize(
