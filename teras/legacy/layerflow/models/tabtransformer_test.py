...
from teras.legacy.layers import CategoricalFeatureEmbedding, Encoder
...
class TabTransformerTest(tf.test.TestCase):
    def setUp(self):
        data = pd.DataFrame({"income": np.ones(10),
                             "goals": np.ones(10),
                             'player_level': [5, 7, 9, 8, 9, 10, 9, 7, 8,
                                              9],
                             'shirt_number': [7, 10, 10, 7, 7, 10, 10, 10,
                                              7, 10]})
        categorical_feats = ["player_level", "shirt_number"]
        numerical_feats = ["income", "goals"]
        self.features_metadata = get_features_metadata_for_embedding(
            pd.DataFrame(data),
        ...
        self.column_embedding = TabTransformerColumnEmbedding(
...
class TabTransformerPretrainerTest(tf.test.TestCase):
    def setUp(self):
        ...
        feature_indices_to_replace = tf.random.uniform(
            (tf.shape(self.data_batch)[0], self.num_features_to_replace),
