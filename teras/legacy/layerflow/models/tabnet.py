...
from teras.legacy.layers.numerical_features_extractor import \
    NumericalFeaturesExtractor
from teras.legacy.losses.tabnet import reconstruction_loss
...
class TabNet(keras.Model):
    ...
    Args:
        ...
        encoder: ``keras.layers.Layer``,
            ...
            If None, an Encoder layer with default values will be used.
                >>> from teras.legacy.layers import TabNetEncoder
    ...
    def __init__(self,
        ...
        categorical_features_exist = len(
            features_metadata["categorical"]) > 0

        if categorical_feature_embedding is None and categorical_features_exist:
            raise ValueError(
                "`categorical_feature_embedding` cannot be None when categorical features exist "
        ...
        if encoder is None:
            raise ValueError(
                "`encoder` cannot be None. Please pass an instance of `TabNetEncoder` layer. "
                "You can import it as, `from teras.legacy.layers import TabNetEncoder``")
        ...
        if categorical_feature_embedding is None:
            ...
            features_embeddings = NumericalFeaturesExtractor(
                features_metadata)(inputs)
        else:
            ...
            features_embeddings = keras.layers.Flatten()(
                features_embeddings)
            if numerical_features_exist:
                numerical_embeddings = NumericalFeaturesExtractor(
                    features_metadata)(inputs)
                features_embeddings = keras.layers.Concatenate(axis=1)(
                    [features_embeddings, numerical_embeddings])
...
class TabNetPretrainer(keras.Model):
    ...
    Args:
        ...
        decoder: ``keras.layers.Layer``,
            ...
            You can import the ``TabNetDecoder`` layer as
                >>> from teras.legacy.layers import TabNetDecoder
    ...
    def __init__(self,
        ...
        self.numerical_features_indices = list(
            self.features_metadata["numerical"].values())
        ...
        self.input_dim = len(self.features_metadata["categorical"]) + len(
            self.features_metadata["numerical"])

        self.binary_mask_generator = tfp.distributions.Binomial(
            total_count=1,
    ...
    def feature_importances_per_sample(self):
        ...
        return tf.concat(self.encoder.feature_importances_per_sample,
                         axis=0)
    ...
    def call(self, inputs, mask=None):
        ...
        encoded_representations = self.model.encoder(encoder_input,
                                                     mask=(1 - mask))
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            if self._categorical_features_exist:
                categorical_embeddings = self.model.categorical_feature_embedding(
                    data)
            ...
            if self._numerical_features_exist:
                if (hasattr(self.model, "numerical_feature_embedding")
                    ...
                    numerical_embeddings = self.model.numerical_feature_embedding(
                        data)
                else:
            ...
            mask = self.binary_mask_generator.sample(
                sample_shape=(batch_size, total_dim))
            tape.watch(mask)
            # Reconstruct samples
        ...
        if len(self.compiled_metrics.metrics) > 0:
            self.compiled_metrics.update_state(embedded_inputs,
