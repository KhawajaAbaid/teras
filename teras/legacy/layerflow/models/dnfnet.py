...
class DNFNet(keras.Model):
    ...
    Args:
        ...
        dnnf_layers: ``List[layers.Layer]`` or ``keras.Model``,
            ...
            You can import the ``DNNF`` layer as follows,
                >>> from teras.legacy.layers import DNNF
    ...
    def __init__(self,
                 ...
                 **kwargs):
        if isinstance(dnnf_layers,
                      (keras.layers.Layer, keras.models.Model)):
            # keep it as is
            ...
            raise ValueError(
                "`dnnf_layers` can either be a list of `DNNF` layers, a Keras Layer, "
    ...
    def get_config(self):
        ...
        config.update({'input_dim': self.input_dim,
                       'dnnf_layers': keras.layers.serialize(
                           self.dnnf_layers),
