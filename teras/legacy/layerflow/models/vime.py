...
from teras.legacy.losses import VimeSelfSupervisedLoss
...
class VimeSelf(keras.Model):
    ...
    Args:
        ...
        encoder: ``keras.layers.Layer``,
            ...
            You can import the ``VimeEncoder`` layer as follows,
                >>> from teras.legacy.layers import VimeEncoder
        ...
        mask_estimator: ``keras.layers.Layer``,
            ...
            You can import the ``VimeMaskEstimator`` layer as follows,
                >>> from teras.legacy.layers import VimeMaskEstimator
        ...
        feature_estimator: ``keras.layers.Layer``,
            ...
            You can import the ``VimeFeatureEstimator`` layer as follows,
                >>> from teras.legacy.layers import VimeFeatureEstimator
    """
    ...
    def __init__(self,
        ...
        if encoder is None:
            raise ValueError(
                "`encoder` cannot be None. You must pass an instance of `VimeEncoder` or any custom layer "
                "that can work in its place. \n"
                "You can import the `VimeEncoder` as `from teras.legacy.layers import VimeEncoder`")

        if mask_estimator is None:
            raise ValueError(
                "`mask_estimator` cannot be None. You must pass an instance of `VimeMaskEstimator` or any "
                "custom layer that can work in its place. \n"
                "You can import the `VimeMaskEstimator` as `from teras.legacy.layers import VimeMaskEstimator`")

        if feature_estimator is None:
            raise ValueError(
                "`feature_estimator` cannot be None. You must pass an instance of `VimeFeatureEstimator` "
    ...
    def get_encoder(self):
        ...
        encoder = keras.models.Model(inputs=self.input,
...
class VimeSemi(keras.Model):
    ...
    def __init__(self,
        ...
        if pretrained_encoder is None:
            raise ValueError(
                "`pretrained_encoder` cannot be None. You must pass an instance of pretrained encoder "
        ...
        if mask_generation_and_corruption is None:
            raise ValueError(
                "`mask_generation_and_corruption` cannot be None. You must pass an instance of "
        ...
        if predictor is None:
            raise ValueError(
                "`predictor` cannot be None. You must pass an instance of `VimePredictor` or any "
                "custom layer that can work in its place. \n"
                "You can import the `VimePredictor` as `from teras.legacy.layers import VimePredictor`")
    ...
    def train_step(self, data):
        ...
        X_unlabeled_batch_encoded = tf.stack(X_unlabeled_batch_encoded,
                                             axis=0)
        ...
        with tf.GradientTape() as tape:
            outputs = self({"labeled": X_batch_encoded,
                           ...
                           training=True)
            yu_loss = self.self_supervised_loss(y_true=None,
                                                y_pred=outputs[
                                                    'yv_hat_logit'])
    ...
    def get_config(self):
        ...
        config.update({'input_dim': self.input_dim,
                       ...
                       'mask_generation_and_corruption': keras.layers.serialize(
