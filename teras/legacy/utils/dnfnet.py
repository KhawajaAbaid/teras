...
def generate_random_mask(input_dim,
                         ...
                         num_formulas=None):
    literals_random_mask = tf.TensorArray(size=0, dynamic_size=True,
                                          dtype=tf.float32)
    formulas_random_mask = tf.TensorArray(size=num_formulas,
                                          dtype=tf.float32)
...
def extension_matrix(v,
    ...
    arr_mat_index = 0
    for i in range(num_formulas):
        n_nodes_in_tree = tf.gather(num_literals_per_formula_arr, i %
                                    tf.shape(num_literals_per_formula_arr)[
                                        0])
...
def create_conjunctions_indicator_matrix(b,
    ...
    array_size = (total_number_of_conjunctions // n_different_depth) * \
                 tf.shape(conjunctions_depth_arr)[0]
    ...
    for i in range(total_number_of_conjunctions // n_different_depth):
        ...
        for d in conjunctions_depth_arr:
            b[
            i * n_literals_in_group + s: i * n_literals_in_group + s + d].assign(
                True)
            s += d
...
def create_formulas_indicator_matrix(c,
    ...
    base = 0
    for i in range(num_formulas):
        n_conjunctions = tf.gather(num_conjunctions_arr,
                                   i % tf.shape(num_conjunctions_arr)[0])
...
def compute_total_number_of_literals(num_formulas,
                                     num_conjunctions_arr,
                                     conjunctions_depth_arr):
    return (tf.reduce_sum(num_conjunctions_arr) * tf.reduce_sum(
        conjunctions_depth_arr) * num_formulas) // (
            tf.shape(conjunctions_depth_arr)[0] *
            tf.shape(num_conjunctions_arr)[0])


@tf.function
def compute_total_number_of_conjunctions(num_formulas,
                                         num_conjunctions_arr):
    return (num_formulas // tf.shape(num_conjunctions_arr)[
        0]) * tf.reduce_sum(num_conjunctions_arr)
...
def compute_num_literals_per_formula(num_conjunctions_arr,
    ...
    for n_conjunctions in num_conjunctions_arr:
        num_literals_per_formula_arr.append(
            (n_conjunctions // tf.shape(conjunctions_depth_arr)[
                0]) * tf.reduce_sum(conjunctions_depth_arr))
...
def tf_matmul_sparse_dense(sparse_A,
                           dense_B):
    return tf.transpose(
        tf.sparse.sparse_dense_matmul(tf.sparse.transpose(sparse_A),
                                      tf.transpose(dense_B)))
