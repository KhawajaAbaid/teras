...
SUPPORTED_CLUSTERING_METHODS = ["Agglomerative", "KMeans",
                                "MiniBatchKMeans", "Spectral",
                                "SpectralBiclustering"]
...
def cluster(x, num_clusters=5, method="kmeans"):
    ...
    if method == "agglomerative":
        cluster_idx = AgglomerativeClustering(
            n_clusters=num_clusters).fit_predict(x)
    elif method == "kmeans":
        cluster_idx = KMeans(n_clusters=num_clusters).fit_predict(x)
    elif method == "minibatchkmeans":
        cluster_idx = MiniBatchKMeans(n_clusters=num_clusters).fit_predict(
            x)
    elif method == "spectral":
        cluster_idx = SpectralClustering(
            n_clusters=num_clusters).fit_predict(x)
    elif method == "spectralbiclustering":
        cluster_idx = SpectralBiclustering(n_clusters=num_clusters).fit(
            x).row_labels_
    else:
        raise ValueError(
            f"The clustering method name passed `{method}` is either not "
