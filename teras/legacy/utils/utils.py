...
from teras.legacy.utils.types import ActivationType
...
def get_normalization_layer(normalization: LayerType) -> layers.Layer:
    ...
    if isinstance(normalization, str):
        ...
        if normalization in ["batchnormalization", "batch"]:
            ...
            raise ValueError(
                f"Normalization type {normalization} is not supported in Keras.")
    elif isinstance(normalization, layers.Layer):
        normalization_layer = normalization
    else:
        raise ValueError(
            f"Invalid Normalization value type. Expected type str or keras.layers.Layer"
    ...
    if isinstance(activation, str):
        ...
        try:
            ...
            if activation == "glu":
                ...
                from teras.legacy.utils.node import sparsemoid
                activation_func = sparsemoid
            else:
                # Otherwise return error
                raise ValueError(
                    f"{activation} function's name is either incorrect "
                    f"or this activation is not offered by either Keras and teras.legacy. ")
...
def get_initializer(initializer: LayerType):
    ...
    if isinstance(initializer, str):
        ...
        try:
            ...
            raise ValueError(
                f"{initializer} function's name is either wrong or this activation is not supported by Keras."
                f"Please contact Teras team, and we'll make sure to add this to teras.legacy.")
    else:
...
def get_features_metadata_for_embedding(dataframe: pd.DataFrame,
    ...
    required by the Categorical and Numerical embedding layers in teras.legacy.
    ...
    if categorical_features is None and numerical_features is None:
        raise ValueError(
            "Both `categorical_features` and `numerical_features` cannot be None at the same time. "
    ...
    for idx, col in enumerate(dataframe.columns):
        if categorical_features is not None:
            if col in categorical_features:
                ...
                categorical_features_metadata.update(
                    {col: (idx, vocabulary)})
...
def convert_dict_to_array_tensor(dict_tensor):
    ...
    if not isinstance(dict_tensor, dict):
        warn(
            "Given tensor is not in dictionary format. Hence no processing will be applied. \n"
...
def serialize_layers_collection(layers_collection: LAYERS_COLLECTION):
    ...
    if isinstance(layers_collection, (list, tuple)):
        ...
        layers_collection_serialized = keras.layers.serialize(
            layers_collection)

    return layers_collection_serialized


def deserialize_layers_collection(
        layers_configs_collection: LAYERS_CONFIGS_COLLECTION):
    """
    ...
    if isinstance(layers_configs_collection, list):
        layers_collection_deserialized = [keras.layers.deserialize(layer)
                                          for layer in
                                          layers_configs_collection]
    else:
        # We assume it's either a dict or None
        layers_collection_deserialized = keras.layers.deserialize(
            layers_configs_collection)
